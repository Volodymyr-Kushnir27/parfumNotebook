...
export default function App() {
  ...
  useEffect(() => {
    // намагаємось підвантажити звіт за датою (якщо є)
    const load = async () => {
      try {
  ...
  const addRow = () => {
    ...
    setItems([...items, { position_no: pos, volume: '', bottle: '', color: '', quantity: 0, price: 0, sum: 0 }])
  }

  const updateRow = (idx:number, changes: Partial<Item>) => {
    setItems(prev => {
      const next = [...prev]
